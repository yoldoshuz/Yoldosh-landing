...
import { ArrowLeft, ChevronRight, Loader2, Star, User } from "lucide-react";
  ...
  if (error || !data?.data?.user) {
    return (
      <div className="min-h-screen bg-emerald-500">
        <div className="p-4">
          <Button variant="ghost" onClick={() => router.back()} className="text-white hover:bg-emerald-600">
  ...
  return (
    <div className="min-h-screen bg-neutral-100">
      ...
      <div className="py-4 px-2 flex items-center justify-between bg-emerald-500">
        <Button variant="ghost" onClick={() => router.back()} className="text-white hover:bg-emerald-600 p-2">
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 left-0 right-0 h-32 w-full bg-emerald-500">
          <svg viewBox="0 0 1440 120" className="absolute bottom-0 w-full h-24" preserveAspectRatio="none">
            <path d="M0,64 C240,100 480,100 720,64 C960,28 1200,28 1440,64 L1440,120 L0,120 Z" fill="#f5f5f5" />
          </svg>
        </div>
        ...
        <div className="relative pt-4 flex flex-col justify-center items-center gap-2">
          <Avatar className="size-32 rounded-full bg-emerald-100 flex items-center justify-center border-emerald-500 border">
            ...
            <AvatarFallback className="bg-emerald-300 font-bold text-white text-xl">{user.firstName[0]}</AvatarFallback>
          </Avatar>
          <h2 className="text-emerald-500 text-xl font-bold text-center">
            {user.firstName} {user.lastName}
            <p className="text-sm text-center">{user.role === "Passenger" ? "Пассажир" : user.role}</p>
          </h2>
        </div>
          ...
          <div className="w-full max-w-3xl px-4">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="px-6 py-4 bg-white rounded-3xl">
                <h3 className="text-neutral-900 font-bold text-base mb-1">Возраст:</h3>
                <p className="text-neutral-600 text-sm">{age ? `${age} лет` : "-"}</p>
              </div>

              {/* Bio */}
              <div className="px-6 py-4 bg-white rounded-3xl">
                <h3 className="text-neutral-900 font-bold text-base mb-1">Биография</h3>
              ...
              >
                <div>
                  <h3 className="text-neutral-900 font-bold text-base mb-1">Оценка Пассажира</h3>
                  <div className="flex items-center gap-2">
                    <span className="text-neutral-400 text-xs">{user.ratingCount} отзыва</span>
                  </div>
                </div>
                <div className="flex items-center justify-center gap-2">
                  <Star className="size-4 fill-amber-400 text-amber-400" />
                  <span className="text-neutral-900 font-semibold">{user.rating.toFixed(1)}</span>
