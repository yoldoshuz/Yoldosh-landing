...
export const Blog = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = usePublicBlogs(
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 pt-12 pb-0">
      {/* Header */}
      <div className="mb-4">
        <h1 className="title-2 text-start!">{t("title")}</h1>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-24 text-muted-foreground">{t("empty")}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map((blog: any) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col flex-1 p-5 gap-3">
                  ...
                  <h2 className="font-semibold text-lg leading-tight line-clamp-2 text-neutral-900 dark:text-white group-hover:text-emerald-600 transition-colors">
                    {blog.title}
                  </h2>
